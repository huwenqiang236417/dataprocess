...
for left , right in zip(clf.predict(x1), y1):
      if left == right:
            count1 += 1
print("饮食预测准确度为：%f" %(float(count1)/len(y1)))
...
for left , right in zip(clf.predict(x2), y2):
      if left == right:
            count2 += 1
print("烟酒预测准确度为：%f" %(float(count2)/len(y2)))
...
for left , right in zip(clf.predict(x3), y3):
      if left == right:
            count3 += 1
print("锻炼预测准确度为：%f" %(float(count3)/len(y3)))
...
for left , right in zip(clf.predict(x4), y4):
      if left == right:
            count4 += 1
print("休闲预测准确度为：%f" %(float(count4)/len(y4)))
...
for left , right in zip(clf.predict(x5), y5):
      if left == right:
            count5 += 1
print("家庭情况预测准确度为：%f" %(float(count5)/len(y5)))
