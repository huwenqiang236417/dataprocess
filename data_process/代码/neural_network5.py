...
clf = MLPClassifier().fit(x_train1,y_train1)
clf.predict(x_test1)
end1 = time.time()
...
for left , right in zip(clf.predict(x_test1), y_test1):
      if left == right:
            count1 += 1
print("饮食预测准确度为：%f" %(float(count1)/len(y_test1)))
...
clf = MLPClassifier().fit(x_train2,y_train2)
clf.predict(x_test2)
end2 = time.time()
...
for left , right in zip(clf.predict(x_test2), y_test2):
      if left == right:
            count2 += 1
print("烟酒预测准确度为：%f" %(float(count2)/len(y_test2)))
...
clf = MLPClassifier().fit(x_train3,y_train3)
clf.predict(x_test3)
end3 = time.time()
...
for left , right in zip(clf.predict(x_test3), y_test3):
      if left == right:
            count3 += 1
print("锻炼预测准确度为：%f" %(float(count3)/len(y_test3)))
...
clf = MLPClassifier().fit(x_train4,y_train4)
clf.predict(x_test4)
end4 = time.time()
...
for left , right in zip(clf.predict(x_test4), y_test4):
      if left == right:
            count4 += 1
print("休闲预测准确度为：%f" %(float(count4)/len(y_test4)))
...
clf = MLPClassifier().fit(x_train5,y_train5)
clf.predict(x_test5)
end5 = time.time()
...
for left , right in zip(clf.predict(x_test5), y_test5):
      if left == right:
            count5 += 1
print("家庭情况预测准确度为：%f" %(float(count5)/len(y_test5)))
